...
def grd2dem(inputMeshFile, inputDataFile, inputShapefile,
    ...
    if fileExtension == '.53':
        ...
        he = pyadcircmodules.HarmonicsOutput(inputDataFile)
        he.read()
        ...
        print(
            ...
            '.tif',
            file=metaDataFile)
        print('number.constituents\t:\t' +
              str(he.numConstituents()), file=metaDataFile)
        for i in range(he.numConstituents()):
            constitNames.append(he.name(i))
            # Print to metadatafile
            print(he.amplitude(i).name() + '.frequency\t:\t' +
                  str(he.amplitude(i).frequency()), file=metaDataFile)
    ...
    if useMeshZ:
        ...
        if isASCIIFile and everdried:
            ...
            for n in range(myMesh.numNodes()):
                # This is the z value of the node
