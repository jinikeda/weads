...
biomass_coefficients = {
    ...
    #,
    # July 9, 2024: Pending the following part because this is not actual observation data
...
########################################################################################################################
# --- GLOBAL PARAMETERS (including reference values for no observation data) ---
...
### Jin to Pete on Jul 9 2024 ###: NEED Kr and RRS inputs for Juvenile_Mangrove  #######################################
...
### Jin to Pete on Jul 9 2024  ###: Check the BTR here which is conflict with above mangrove ###########################
...
# F U N C T I O N
#----------------------------------------------------------
def read_band_value(raster_file, band_num): # Future modify the function read transform and projection information March 11, 2024 (Jin)
...
def create_raster(file, rasterHC, zarray, dtype, no_data_value,stats_flag=False):
    ...
    out_ds = gtiff_driver.Create(file, rasterHC.RasterXSize, rasterHC.RasterYSize, rasterHC.RasterCount, dtype) # dtype is e.g. gdal.GDT_Int32 and gdal.GDT_Float32
...
def mem(inputRasterHyControl, inputRasterTopoBathy, inputRasterTidalDatumsIDW,vegetationFile, outputRaster, deltaT = 5):
    ...
    if vegetationFile==None:
        ...
        a3, b3, c3 = calculate_coefficients(Dmin_fmarsh, Dmax_fmarsh, Dopt_fmarsh, Bmax_fmarsh)  # 3: Irregularly flooded marsh (NWI = 20)
...
########################################################################################################################
    ...
    submergence_mask = water_mask | intertidal_mask | (2.5 < hc)   # submergence region (hc = 1.0, 2.0, 3.0 including lake and pond)
    ...
    if vegetationFile == None:
        ...
        print ('Irregular_depth [cm], min, max, mean, std', irregular_NWI_depth.min(), irregular_NWI_depth.max(), irregular_NWI_depth.mean(), irregular_NWI_depth.std())
        ################################################################################################################
        ...
        mask_mangrove = mask_mangrove & (0 < B2)  # & (B2 <= PH2) # This part keep for future revision (0 < B2) & (B2 <= PH2)
...
########################################################################################################################
        ...
        B1_masked = np.where((mask_salt_marsh | mask_mangrove), B1, ndv) # mask_marsh may compete with mangrove due to the dilution order, so evaluate both masks here.
