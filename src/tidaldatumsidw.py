...
def tidaldatumsidw(inputRasterHyControl,inputRasterTidalDatums,\
    ...
    mlwIDW=np.ones((rasterTD.RasterYSize,rasterTD.RasterXSize),dtype=float); mlwIDW=mlwIDW*ndv;
    mslIDW=np.ones((rasterTD.RasterYSize,rasterTD.RasterXSize),dtype=float); mslIDW=mslIDW*ndv;
    mhwIDW=np.ones((rasterTD.RasterYSize,rasterTD.RasterXSize),dtype=float); mhwIDW=mhwIDW*ndv;
    X=np.zeros((rasterHC.RasterYSize,rasterHC.RasterXSize),dtype=float)
    x=np.ones((1,rasterHC.RasterXSize),dtype=float); x=np.cumsum(x,axis=1); X=X+x;
    Y=np.zeros((rasterHC.RasterYSize,rasterHC.RasterXSize),dtype=float)
    y=np.ones((rasterHC.RasterYSize,1),dtype=float); y=np.cumsum(y,axis=0); Y=Y+y;
    
    # --- PERFORM IDW OF TIDAL DATUMS ---
    #print ("")
    #print ("Performing IDW of tidal datums")
    
    np.seterr("ignore") #supress/hide: invalid value encountered in true_divide
    
    # Gathering x,y,z point cloud from the raster
    translator = lio.Translator("geotiff", "np")
    point_cloud = translator.translate(inputRasterTidalDatums,no_data = ndv, band = 1)
        ...
        for kk in range(0,rasterHC.RasterYSize):
            if (-0.5 <= hc[kk][k]) & (hc[kk][k] <= 1.5): # Only interpolate across the LAND (0) and intertidal (1) regions
                ...
                for i in range(0,numNeighbors):
                    ...
                    c, r = src.raster.coord2pixel(pts[ii[i]][0],pts[ii[i]][1],rasterTD)
...
########################################################################################################################
    ...
    dst_ds=driver.Create(outputRaster,rasterHC.RasterXSize,rasterHC.RasterYSize,3,dst_datatype)
    dst_ds.SetGeoTransform(dst_geot); dst_ds.SetProjection(dst_proj.ExportToWkt());
    dst_ds.GetRasterBand(1).SetNoDataValue(ndv); dst_ds.GetRasterBand(1).WriteArray(mlwIDW);
    dst_ds.GetRasterBand(2).SetNoDataValue(ndv); dst_ds.GetRasterBand(2).WriteArray(mslIDW);
    dst_ds.GetRasterBand(3).SetNoDataValue(ndv); dst_ds.GetRasterBand(3).WriteArray(mhwIDW);
