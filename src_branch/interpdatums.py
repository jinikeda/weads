...
pts=np.ones((numPoints,16),dtype=float); myFile.seek(0);
for j in tqdm(range(numPoints)):
    ...
    pts[j][14]=float("NaN"); pts[j][15]=float("NaN");
myFile.close()
...
for j in range(numPoints):
    ...
    if pts[j][8]>0.0:
        listW.append(pts[j][:])
arrayI=np.array(listI); arrayW=np.array(listW);
distI=np.ones((len(listI),3),dtype=float); distW=np.ones((len(listW),3),dtype=float);
mlwI=np.ones((len(listI),3),dtype=float); mhwI=np.ones((len(listI),3),dtype=float);
for j in tqdm(range(len(listI))):
    distW[:,0]=(arrayI[j][4]-arrayW[:,4])**2.0; distW[:,1]=(arrayI[j][5]-arrayW[:,5])**2.0;
    distW[:,2]=(distW[:,0]+distW[:,1])**0.5; arrayW[:,14]=distW[:,2];
    arrayW=arrayW[np.argsort(arrayW[:,14])]
    distI[j][0]=arrayW[0][14]; distI[j][1]=arrayW[1][14]; distI[j][2]=arrayW[2][14];
    distI[j][0]=1.0/distI[j][0]**2.0; distI[j][1]=1.0/distI[j][1]**2.0; distI[j][2]=1.0/distI[j][2]**2.0;
    #--- Mean low water ---
    mlwI[j][0]=arrayW[0][11]; mlwI[j][1]=arrayW[1][11]; mlwI[j][2]=arrayW[2][11];
    numer=distI[j][0]*mlwI[j][0]+distI[j][1]*mlwI[j][1]+distI[j][2]*mlwI[j][2]
    denom=distI[j][0]+distI[j][1]+distI[j][2]
    arrayI[j][14]=numer/denom
    #--- Mean high water ---
    mhwI[j][0]=arrayW[0][13]; mhwI[j][1]=arrayW[1][13]; mhwI[j][2]=arrayW[2][13];
...
#    myOut.write(str(int(arrayI[j][0]))+" "); myOut.write(str(int(arrayI[j][1]))+" ");
    myOut.write(str(j+1)+" "); myOut.write(str(int(arrayI[j][1]))+" ");
    myOut.write(str(arrayI[j][2])+" "); myOut.write(str(arrayI[j][3])+" ");
    myOut.write(str(arrayI[j][4])+" "); myOut.write(str(arrayI[j][5])+" ");
    myOut.write(str(arrayI[j][6])+" "); myOut.write(str(arrayI[j][7])+" ");
    myOut.write(str(arrayI[j][8])+" "); myOut.write(str(arrayI[j][9])+" ");
    myOut.write(str(arrayI[j][10])+" "); myOut.write(str(arrayI[j][11])+" ");
    myOut.write(str(arrayI[j][12])+" "); myOut.write(str(arrayI[j][13])+" ");
    myOut.write(str(arrayI[j][14])+" "); myOut.write(str(arrayI[j][15])+"\n")
myOut.close()
