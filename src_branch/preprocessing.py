...
########################################################################################################################
#### Jin's note need to add shaple file to extract the domain.
# inputShapeFile
#
# June 20, 2024
...
myRow=myLine.split();
bbox[0]=float(myRow[0]);
bbox[1]=float(myRow[1]);
myLine=myFile.readline();
myRow=myLine.split();
bbox[2]=float(myRow[0]);
bbox[3]=float(myRow[1]);
myFile.close()
...
for j in range(numAttributes):
    ...
    if myRow[0]=="mannings_n_at_sea_floor":
        ...
        myLine=myFile.readline()
        myLine=myFile.readline()
mann=np.ones((numNodes,1),dtype=float);
mann=mann*mann0; #(Jin's comments why?)
...
########################################################################################################################
# This code have to replace to inundationTime.63
# Short Comments
# Jin June 20, 2024
# Also, need to consider to read netcdf file. insall netCDF4, xarray?
########################################################################################################################
...
myOut=open("harmonics.freq","w");
myLine=myFile.readline();
myRow=myLine.split();
numHarm=int(myRow[0]);
harmAMP=np.ones((numNodes,numHarm),dtype=float);
harmPHA=np.ones((numNodes,numHarm),dtype=float);
for j in range(numHarm):
    myLine=myFile.readline();
    myRow=myLine.split();
    myOut.write(str(myRow[0])+"\n");
myLine=myFile.readline()
for j in tqdm(range(numNodes)):
    myLine=myFile.readline()
    for jj in range(numHarm):
        myLine=myFile.readline();
        myRow=myLine.split();
        harmAMP[j][jj]=float(myRow[0]);
        harmPHA[j][jj]=float(myRow[1]);
myFile.close()

#--- Filter for bounding box ---
print("\n"); print("   Filtering data for bounding box...\n");
myOut1=open("filteredData.pts","w")
myOut2=open("filteredHarmAmp.pts","w");
myOut3=open("filteredHarmPha.pts","w");
cnt=0
for j in tqdm(range(numNodes)):
    if nx[j][0]>bbox[0] and nx[j][0]<bbox[2]:
        if ny[j][0]>bbox[1] and ny[j][0]<bbox[3]:
            cnt=cnt+1
            myOut1.write(str(cnt)+" ");
            myOut1.write(str(j+1)+" ");
            myOut1.write(str(nx[j][0])+" ");
            myOut1.write(str(ny[j][0])+" ");
            myOut1.write(str(nxcpp[j][0])+" "); # not sure Jin June 20,2024
            myOut1.write(str(nycpp[j][0])+" "); # not sure
            myOut1.write(str(nz[j][0])+" ");
            myOut1.write(str(mann[j][0])+" "); #manning?
            myOut1.write(str(ed[j][0])+" "); # where is this variable?
            myOut1.write(str(it[j][0])+" ");
            myOut1.write(str(mi[j][0])+"\n") # where is this variable?
            for jj in range(numHarm-1):
                myOut2.write(str(harmAMP[j][jj])+" ");
                myOut3.write(str(harmPHA[j][jj])+" ");
